...
import {
  getLastStockHistoricalEntryTime,
  saveStock,
  saveStockDetail,
} from "./dbService.ts";
...
function scheduleOnHourlyInterval() {
  ...
  console.log(
    `Scheduling stock price update in ${
      Math.round(msUntilNextHour / 1000)
    } seconds (next hour mark)`,
  );
  ...
  const taskId = setTimeout(() => {
    ...
    console.log(
      `Stock price updates now scheduled to run on the hour, every hour`,
    );
  }, msUntilNextHour);
...
function scheduleAtMidnightEST() {
  ...
  scheduledTasks.push(taskId);
  console.log(
    `Stock details update scheduled at midnight EST (in ${
      Math.round(msUntilMidnight / 60000)
    } minutes)`,
  );
}
...
async function checkAndRunScheduledTasks() {
  ...
  if (!lastRunTime || (now.getTime() - lastRunTime.getTime() > ONE_HOUR)) {
    console.log(
      "No price updates in the last hour. Running stock price update now...",
    );
    updateAllStockPrices();
  }
  ...
  if (!lastRunTime || (now.getTime() - lastRunTime.getTime() > ONE_DAY)) {
    console.log(
      "No detailed updates in the last 24 hours. Running stock details update now...",
    );
    updateAllStockDetails();
  }
}
...
async function updateAllStockPrices() {
  try {
    console.log(
      `[${new Date().toISOString()}] Starting scheduled stock price update...`,
    );
    ...
    console.log(
      ...
      }] Completed stock price update. Successfully updated ${successCount}/${stocks.length} stocks.`,
    );
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Failed to update stock prices:`,
      error,
    );
  }
}
...
async function updateAllStockDetails() {
  try {
    console.log(
      `[${
        new Date().toISOString()
      }] Starting scheduled stock details update...`,
    );
    const stocks = await fetchAllStocks();
    let successCount = 0;
    ...
    for (const stock of stocks) {
      try {
        ...
        console.error(
          `Error updating stock details for ${stock.ticker}:`,
          error,
        );
      }
    ...
    console.log(
      `[${
        new Date().toISOString()
      }] Completed stock details update. Successfully updated ${successCount}/${stocks.length} stocks.`,
    );
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Failed to update stock details:`,
      error,
    );
  }
}
