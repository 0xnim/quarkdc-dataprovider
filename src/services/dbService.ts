...
import { Stock, StockDetail, StockHistorical, ShareholderHistorical } from "../models/types.ts";
...
export async function getStockHistoricalPrices(
  ...
  `;
    ...
    if (startDate) {
        query += " AND sh.recorded_at >= timezone('America/New_York', $2::timestamp)";
        params.push(startDate);

        if (endDate) {
            query += " AND sh.recorded_at <= timezone('America/New_York', $3::timestamp)";
            params.push(endDate);
        }
    }
...
export async function getShareholderHistorical(
  ...
  `;
    ...
    if (startDate) {
        query += ` AND sh.recorded_at >= timezone('America/New_York', $${paramIndex}::timestamp)`;
        params.push(startDate);
        paramIndex++;
